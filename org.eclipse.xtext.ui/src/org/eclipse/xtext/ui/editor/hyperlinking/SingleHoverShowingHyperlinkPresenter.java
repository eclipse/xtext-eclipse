...
public class SingleHoverShowingHyperlinkPresenter implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		if (method.getName().startsWith("showHyperlinks") && (args.length >= 1) && (args[0] instanceof IHyperlink[])) {
			final List<IHyperlink> nullsafe = makeNullsafe(Arrays.asList((IHyperlink[]) args[0]));
			if (!nullsafe.isEmpty()) {
				args[0] = nullsafe;
				final Object result = method.invoke(delegate, args);
				final List<IHyperlink> activeHyperlinks = nullsafe;
				if (activeHyperlinks.size() == 1) {
					final IHyperlink singleHyperlink = activeHyperlinks.get(0);
					if (SHOW_ALWAYS == singleHyperlink.getTypeLabel()) {
						final int start = singleHyperlink.getHyperlinkRegion().getOffset();
						final int end = start + singleHyperlink.getHyperlinkRegion().getLength();
	...
	protected List<IHyperlink> makeNullsafe(List<IHyperlink> hyperlinks) {
		if (hyperlinks.stream().anyMatch(hyperLink -> (hyperLink == null || hyperLink.getHyperlinkRegion() == null))) {
			final List<IHyperlink> nullSafeList = new ArrayList<>();
